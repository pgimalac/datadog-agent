...
func NewTester(tt *testing.T, vm client.VM, opts ...TesterOption) (*Tester, error) {
	t := &Tester{}

	var err error

	t.vm = vm
	t.InstallTestClient = common.NewWindowsTestClient(tt, t.vm)
	t.hostInfo, err = windows.GetHostInfo(t.vm)
	if err != nil {
		return nil, err
	}
...
func (t *Tester) testAgentCodeSignature(tt *testing.T) bool {
	...
	return windowsAgent.TestValidDatadogCodeSignatures(tt, t.vm, paths)
}
...
func (t *Tester) testDoesNotChangeSystemFiles(tt *testing.T) bool {
	return tt.Run("does not remove system files", func(tt *testing.T) {
		...
		output, err := t.vm.ExecuteWithError(cmd)
...
func (t *Tester) InstallAgentPackage(tt *testing.T, agentPackage *windowsAgent.Package, args string, logfile string) (string, error) {
	...
	remoteMSIPath, err := windows.GetTemporaryFile(t.vm)
	require.NoError(tt, err)
	err = windows.PutOrDownloadFile(t.vm, agentPackage.URL, remoteMSIPath)
	require.NoError(tt, err)
	...
	err = windows.InstallMSI(t.vm, remoteMSIPath, args, logfile)
...
func (t *Tester) TestInstallAgentPackage(tt *testing.T, agentPackage *windowsAgent.Package, args string, logfile string) bool {
	return tt.Run("install the agent", func(tt *testing.T) {
		...
		windowsAgent.TestValidDatadogCodeSignatures(tt, t.vm, []string{remoteMSIPath})
		common.CheckInstallation(tt, t.InstallTestClient)
		t.testAgentCodeSignature(tt)

		tt.Run("agent version", func(tt *testing.T) {
			installedVersion, err := t.InstallTestClient.GetAgentVersion()
			require.NoError(tt, err, "should get agent version")
			assert.Equal(tt, t.expectedAgentVersion, installedVersion, "installed agent version should match expected agent version")
		})
	})
}

// TestUninstall uninstalls the agent and runs tests
func (t *Tester) TestUninstall(tt *testing.T) bool {
	return tt.Run("uninstall the agent", func(tt *testing.T) {
		err := windowsAgent.UninstallAgent(t.vm, "uninstall.log")
