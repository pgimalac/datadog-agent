...
func (p *Processor) Process(ctx ProcessorContext, list interface{}) (processResult ProcessResult, processed int) {
	...
	defer orchestrator.RecoverOnPanic()
	...
	for _, resource := range resourceList {
		...
		if err != nil {
			log.Warnc(orchestrator.NewMarshallingError(err).Error(), orchestrator.ExtraLogContext...)
			continue
		}
...
func ChunkManifest(ctx ProcessorContext, buildManifestBody func(ctx ProcessorContext, resourceManifests []interface{}, groupSize int) model.MessageBody, resourceManifestModels []interface{}) []model.MessageBody {
	...
	chunks := orchestrator.ChunkOrchestratorPayloadsBySizeAndWeight(resourceManifestModels, resourceManifestModels, weightForOrchestratorPayload, ctx.GetOrchestratorConfig().MaxPerMessage, ctx.GetOrchestratorConfig().MaxWeightPerMessageBytes)
...
func ChunkMetadata(ctx ProcessorContext, p *Processor, resourceMetadataModels, resourceManifestModels []interface{}) []model.MessageBody {
	...
	chunks := orchestrator.ChunkOrchestratorPayloadsBySizeAndWeight(resourceMetadataModels, resourceManifestModels, weightForOrchestratorPayload, ctx.GetOrchestratorConfig().MaxPerMessage, ctx.GetOrchestratorConfig().MaxWeightPerMessageBytes)
