...
func (m *Span) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.SemanticsSchemaUrl) > 0 {
		i -= len(m.SemanticsSchemaUrl)
		copy(dAtA[i:], m.SemanticsSchemaUrl)
		i = encodeVarint(dAtA, i, uint64(len(m.SemanticsSchemaUrl)))
		i--
		dAtA[i] = 0x72
	}
...
func (m *Span) SizeVT() (n int) {
	...
	l = len(m.SemanticsSchemaUrl)
	if l > 0 {
		n += 1 + l + sov(uint64(l))
	}
	if len(m.SemanticTags) > 0 {
		for k, v := range m.SemanticTags {
			_ = k
			_ = v
...
func (m *Span) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 14:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field SemanticsSchemaUrl", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.SemanticsSchemaUrl = string(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		case 15:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field SemanticTags", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if m.SemanticTags == nil {
				m.SemanticTags = make(map[string]uint64)
			}
			var mapkey string
			var mapvalue uint64
			for iNdEx < postIndex {
			...
			m.SemanticTags[mapkey] = mapvalue
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := skip(dAtA[iNdEx:])
