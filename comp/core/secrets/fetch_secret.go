...
func execCommand(secret *secretResolver, inputPayload string) ([]byte, error) {
	ctx, cancel := context.WithTimeout(context.Background(),
		time.Duration(secret.backendTimeout)*time.Second)
	defer cancel()

	cmd, done, err := commandContext(ctx, secret.backendCommand, secret.backendArguments...)
	if err != nil {
		return nil, err
	}
	defer done()

	if err := checkRights(cmd.Path, secret.commandAllowGroupExec); err != nil {
		return nil, err
	}
	...
	log.Debugf("%s | secret_backend_command '%s' completed in %s", time.Now().String(), secret.backendCommand, elapsed)
	...
	if err != nil {
		...
		tlmSecretBackendElapsed.Add(float64(elapsed.Milliseconds()), secret.backendCommand, exitCode)

		if ctx.Err() == context.DeadlineExceeded {
			return nil, fmt.Errorf("error while running '%s': command timeout", secret.backendCommand)
		}
		return nil, fmt.Errorf("error while running '%s': %s", secret.backendCommand, err)
	}

	log.Debugf("secret_backend_command stderr: %s", stderr.buf.String())

	tlmSecretBackendElapsed.Add(float64(elapsed.Milliseconds()), secret.backendCommand, "0")
...
func fetchSecret(secret *secretResolver, secretsHandle []string) (map[string]string, error) {
	...
	output, err := runCommand(secret, string(jsonPayload))
	if err != nil {
		return nil, err
	}
	...
	for _, sec := range secretsHandle {
		...
		if secret.removeTrailingLinebreak {
